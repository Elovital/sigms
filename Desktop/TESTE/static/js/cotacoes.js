...
const CORES = [
  ...
  { primary: '#c27803', light: '#fdf6b2', name: 'yellow' },
];
...
export async function renderCotacoes(container) {
  ...
  // Inicializa 4 seguradoras vazias
  state.seguradoras = [1, 2, 3, 4].map((_, i) => newSeg(i));
...
<div class="page-content" id="cotacoes-root">
  ...
  <!-- Cards de Input das 4 Seguradoras -->
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px" id="seg-cards">
    ${[0,1,2,3].map(i => buildSegCard(i, segList)).join('')}
  </div>
...
<div class="card" style="border-top:3px solid ${cor.primary};min-width:0">
  ...
  <div class="card-body" style="padding:16px">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">
      <div>
        <label style="font-size:11px;font-weight:600;color:var(--gray-600);text-transform:uppercase;letter-spacing:.4px;display:block;margin-bottom:4px">⏳ Carência (dias)</label>
        <input type="number" class="form-control" id="seg-carencia-${i}" placeholder="Ex: 90" min="0" style="font-size:13px">
      </div>
      <div>
        <label style="font-size:11px;font-weight:600;color:var(--gray-600);text-transform:uppercase;letter-spacing:.4px;display:block;margin-bottom:4px">🌐 Co-pag. Fora Rede (%)</label>
        <input type="number" class="form-control" id="seg-copag-fora-${i}" placeholder="Ex: 30" min="0" max="100" style="font-size:13px">
      </div>
    </div>
...
function bindEvents(segList) {
  ...
  [0,1,2,3].forEach(i => {
    ...
    document.getElementById(`seg-carencia-${i}`).addEventListener('input',   () => { readSeg(i); updateAll(); });
    document.getElementById(`seg-copag-fora-${i}`).addEventListener('input', () => { readSeg(i); updateAll(); });
  ...
  document.getElementById('btn-limpar').addEventListener('click', () => {
    [0,1,2,3].forEach(i => {
      ...
      document.getElementById(`seg-carencia-${i}`).value = '';
      document.getElementById(`seg-copag-fora-${i}`).value = '';
      COBERTURAS.forEach(c => {
...
function readSeg(i) {
  ...
  s.carencia       = parseInt(document.getElementById(`seg-carencia-${i}`).value) || 0;
  s.copag_fora_rede = parseFloat(document.getElementById(`seg-copag-fora-${i}`).value) || 0;
  COBERTURAS.forEach(c => {
    s.coberturas[c.id] = {
...
function updateTabela() {
  ...
  const carencias = segs.map((s, i) => {
    if (!s.carencia) return `<td style="text-align:center;color:var(--gray-400);font-size:13px">—</td>`;
    return `<td style="text-align:center;background:${CORES[i].light};padding:8px">
      <span style="font-size:13px;font-weight:700;color:${CORES[i].primary}">${s.carencia} dias</span>
    </td>`;
  }).join('');

  const copagsFora = segs.map((s, i) => {
    if (!s.copag_fora_rede) return `<td style="text-align:center;color:var(--gray-400);font-size:13px">—</td>`;
    return `<td style="text-align:center;background:${CORES[i].light};padding:8px">
      <span style="font-size:13px;font-weight:700;color:${CORES[i].primary}">${s.copag_fora_rede}%</span>
    </td>`;
  }).join('');
  ...
  <table style="width:100%;border-collapse:collapse">
    <thead>
      <tr style="background:var(--gray-800)">
        ...
        ${labels.map((l, i) => `
        <th style="padding:12px 8px;font-size:12px;font-weight:600;color:white;text-align:center;width:18.75%;border-left:1px solid rgba(255,255,255,.1)">
    ...
    <tfoot>
      <tr style="background:var(--gray-100)">
        <td style="padding:10px 14px;font-size:12px;font-weight:700;color:var(--gray-700);text-transform:uppercase">⏳ Período de Carência</td>
        ${carencias}
      </tr>
      <tr style="background:var(--gray-50)">
        <td style="padding:10px 14px;font-size:12px;font-weight:700;color:var(--gray-700);text-transform:uppercase">🌐 Co-pag. Fora da Rede</td>
        ${copagsFora}
      </tr>
      <tr style="background:var(--gray-100)">
